...
class BibleRepository(context: Context) {
    ...
    suspend fun setBookmark(verseId: Int, bookmarked: Boolean) = withContext(Dispatchers.IO) {
        db.execSQL(
            ...
            arrayOf<Any>(if (bookmarked) 1 else 0, if (bookmarked) System.currentTimeMillis() else 0, verseId),
        )
    }
    ...
    suspend fun setHighlight(verseId: Int, colorIndex: Int) = withContext(Dispatchers.IO) {
        db.execSQL(
            ...
            arrayOf<Any>(colorIndex, if (colorIndex > 0) System.currentTimeMillis() else 0, verseId),
        )
    }
    ...
    suspend fun setNote(verseId: Int, note: String?) = withContext(Dispatchers.IO) {
        db.execSQL(
            ...
            arrayOf<Any?>(note, if (!note.isNullOrBlank()) System.currentTimeMillis() else 0, verseId),
        )
    }
